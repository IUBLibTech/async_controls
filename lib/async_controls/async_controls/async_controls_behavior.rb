...
module AsyncControls
  module AsyncControlsBehavior
    ...
    def show
      ...
      @async_controls_presenter = AsyncControls::AsyncControlsPresenter.new(storage_proxy_response, file_set_solr_document)
      super
    end
    ...
    def file_set_solr_document
      ...
      @file_set_solr_document ||= presenter.solr_document
      # @file_set_solr_document ||= curation_concern_type.load_instance_from_solr(params[:id])
    end

    def filename
      # A FileSet's label should be the actual filename.
      @filename ||= File.basename(file_set_solr_document.label)
    end
    ...
    def storage_proxy
      @storage_proxy ||= AsyncControls::StorageProxyClient.new
    end
